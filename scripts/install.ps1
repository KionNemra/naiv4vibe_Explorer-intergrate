...
if (-not $runningAsAdmin) {
  Write-Warning "PowerShell is not running as Administrator. COM registration may fail with access denied."
}
...
if ($exitCode -ne 0) {
  ...
  throw "regsvr32 failed with exit code $exitCode. $commonHint`nManual check: `"$regsvr32`" `"$resolvedDllPath`""
}
